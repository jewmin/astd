			...
			else
			{
				...
				else
				{
                    ...
                    if (arch_trade)
					{
						...
						{
							int map_count = 0;
							long num6 = (long)miscManager.handleNewTradeInfo(this._proto, this._logger, this._user, visit_merchants, max_fail_count, base.getSilverAvailable(), base.getGoldAvailable(), out map_count);
					...
					foreach (int current in this._movableSerial)
					{
                        ...
                        else if (current == 1)
                        {
                            ...
                            {
                                int num30 = 0;
                                long num31 = (long)miscManager.handleNewTradeInfo(this._proto, this._logger, this._user, visit_merchants, max_fail_count, base.getSilverAvailable(), base.getGoldAvailable(), out num30);
                                if (num31 == 10L)
                                {
                                    result = base.next_halfhour();
                                    return result;
