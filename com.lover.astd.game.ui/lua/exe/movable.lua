...
MovableExe.execute = function()
  ...
  -- 纺织布匹
  if MovableExe.weave_like == nil then
    local weave_like = GetDbConfig(ConfigStrings.weave_item)
    if weave_like ~= nil and weave_like ~= "" then
      MovableExe.weave_like = tonumber(weave_like)
    else
      MovableExe.weave_like = 0
    end
  end
  -- 精炼
  ...
  for i = 1, #movable_order do
      ...
      local result, weave_state
  		result, weave_state, MovableExe.weave_like = MiscManager():handleWeaveInfo(ProtocolMgr(), ILogger(), User(), weave_price, weave_count, MovableExe.weave_like, true, false, false)
      -- ILogger():logInfo(string.format("布匹_%d", MovableExe.weave_like))
